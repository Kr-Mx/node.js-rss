...
router.route('/').post(async (req, res, next) => {
  try {
    const task = await tasksService.createTask({ ...req.body, boardId: req.params.id });
...
router.route('/:taskId').get(async (req, res, next) => {
  try {
    const task = await tasksService.getById(req.params.id, req.params.taskId);
    task ? await res.status(200).json(Task.toResponse(task)) : res.status(404).send('Task not found');
  } catch (e) {
    if (e instanceof ErrorHandler) {
      return handleError(e, res);
...
router.route('/:taskId').delete(async (req, res, next) => {
  try {
    const task = await tasksService.deleteTask(req.params.id, req.params.taskId);
